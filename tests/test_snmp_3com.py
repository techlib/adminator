...
agent = SNMP3comAgent(None, None, 0, 0)
...
def test_process_speed():
    assert agent.process_speed('1') == None
    assert agent.process_speed(speed_1M) == 1
    assert agent.process_speed(speed_10M) == 10
    assert agent.process_speed(speed_100M) == 100
    assert agent.process_speed(speed_1G) == 1000

def test_process_speed_special():
    assert agent.process_speed(speed_special) == 24000
